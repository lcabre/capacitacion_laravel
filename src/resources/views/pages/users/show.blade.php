...
{{--                                        <option value="{{$role->id}}" {{(collect(old('roles'))->contains($role->id)) ? 'selected':''}} {{$role->user_id && old('roles') == NULL ? 'selected':''}}>{{ $role->nombre }}</option>--}}
                                        <option value="{{$role->id}}" {{ $user->roles->contains('id', $role->id)?'selected':'' }}>{{ $role->nombre }}</option>
                                    @endforeach
                                </select>
                                @error('roles')
                                <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('projects') is-invalid @enderror" multiple data-live-search="true" name="projects[]">
                                    @foreach($projects as $project)
                                        <option value="{{$project->id}}" {{ $user->projects->contains('id',$project->id) ? 'selected':'' }}>{{ $project->nombre }}</option>
                                    @endforeach
                                </select>
                                @error('projects')
                                <div class="invalid-feedback">{{ $message }}</div>
